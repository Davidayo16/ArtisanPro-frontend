...
export default function ProfileWizard() {
  ...
  // Services & Categories from backend
   ...
   const fetchData = async () => {
     try {
       ...
       if (categoriesRes.success && Array.isArray(categoriesRes.data)) {
         ...
         console.log("CATEGORIES SET:", categoriesRes.data.length);
       }

       if (Array.isArray(servicesRes)) {
         setServices(servicesRes);
         console.log("SERVICES SET:", servicesRes.length);
       }
     } catch (error) {
     ...
     if (currentStep === 1) {
       await artisanApi.updateBasicInfo({
         ...
         location: {
           ...
           coordinates: formData.location.coordinates, // ✅ This now has real coords!
         },
       });
       ...
       // Get unique category IDs from selected services
       const categoryIds = [
         ...new Set(
           selectedServices
             .map((s) => s.category?._id || s.category) // Handle both populated and non-populated
             .filter(Boolean)
         ),
       ];

       console.log("📦 Saving Services & Categories:", {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Fetch fresh profile status
    const freshUserData = await authApi.getProfileStatus();
    console.log("📊 Profile Status Response:", freshUserData);

    const profileData = freshUserData.data || freshUserData;
    console.log("📋 Profile Data:", profileData);

    // 🔥 USE THE NEW setProfileStatus FUNCTION
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {currentStep === 2 && (
            <div className="space-y-6">
              ...
              {/* Services Grid */}
              <div>
                <div className="grid grid-cols-2 gap-3 max-h-96 overflow-y-auto p-2 border-2 border-gray-200 rounded-xl">
