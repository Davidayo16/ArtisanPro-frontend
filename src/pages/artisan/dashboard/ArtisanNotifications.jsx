// frontend/pages/artisan/ArtisanNotifications.jsx
...
export default function ArtisanNotifications() {
  ...
  const jobNotifCount = notifications.filter((n) =>
    n.type?.includes("booking")
  ).length;

  const earningsNotifCount = notifications.filter(
    (n) => n.type === "payment_received" || n.type === "payout_processed"
  ).length;

  const reviewNotifCount = notifications.filter((n) =>
  ...
  const getFilteredNotifications = () => {
    ...
    if (activeTab !== "all") {
      if (activeTab === "unread") {
        ...
        list = list.filter((n) => n.type?.includes("review"));
      }
    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="mt-1" style={{ color: COLORS.gray[600] }}>
            Stay on top of jobs, payments, and reviews
          </p>
        </div>
        <button
          onClick={() => navigate("/artisan/dashboard/settings")}
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 lg:pb-0">
            {[
              ...
              { id: "job_request", label: "Jobs", count: jobNotifCount },
              { id: "payment", label: "Earnings", count: earningsNotifCount },
              { id: "review", label: "Reviews", count: reviewNotifCount },
            ].map((tab) => (
              <button
                key={tab.id}
